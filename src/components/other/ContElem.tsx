...
import { FunctionContext } from 'components/Table/Table'
import { UserContext } from 'components/other/ElemBox'

const ContElem = () => {
  const { user, board } = useContext(UserContext)
  const { onClick } = useContext(FunctionContext)
    ...
    return (
      <>
        ...
        </div>
        {
          board.id === 2 &&
          <div
            className="d-flex justify-content-center align-items-center good-border ml-0 "
            style={{ width: '50px' }}
          >
